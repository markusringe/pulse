...
/* =============================================================================
   Kompakter QR-Code-Encoder (Byte-Modus, Version 1–6, ECC M)
   Keine externe Bibliothek — Bild wird auf ein Canvas gerastert.
   ============================================================================= */
...
function penalty(bits, size) {
  let score = 0;
  const at = (x, y) => bits[y * size + x];
  ...
  let dark = 0;
  for (let i = 0; i < bits.length; i++) dark += bits[i];
  score += Math.abs(Math.floor((dark * 100) / bits.length / 5) - 10) * 10;
  return score;
}
...
function encodeQr(text) {
  ...
  for (let mask = 0; mask < 8; mask++) {
    ...
    const score = penalty(matrix.bits, matrix.size);
    if (score < bestScore) {
      bestScore = score;
      best = matrix;
    }
