...
async function ensureSendmailForPulse(opts = {}) {
  /* Test-/CI-Umgebung: keine Auto-Konfiguration (würde provider=none überschreiben). */
  if (
    process.env.NODE_ENV === "test" ||
    /^(0|false|off|no|disabled)$/i.test(String(process.env.PULSE_SKIP_SENDMAIL_SETUP || "").trim())
  ) {
