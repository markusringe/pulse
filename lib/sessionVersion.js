...
const HIGH_PRIORITY_PARTICIPANT_EVENTS = new Set(["slide", "deck", "lobby"]);
...
/**
 * Teilnehmer/Stage: Folienwechsel nie wegen veralteter stateVersion blockieren.
 * @param {{ stateVersion?: number } | null | undefined} session
 * @param {{ stateVersion?: number } | null | undefined} payload
 * @param {{ role?: string, eventType?: string }} [opts]
 * @returns {boolean}
 */
function acceptStructural(session, payload, { role = "presenter", eventType = "" } = {}) {
  const r = String(role || "");
  if ((r === "join" || r === "participant" || r === "stage") && HIGH_PRIORITY_PARTICIPANT_EVENTS.has(String(eventType || ""))) {
    return true;
  }
 ...
 * @param {{ interaction?: { seq?: number } } | null | undefined} slide
 * @returns {number}
 */
function interactionSeq(slide) {
 ...
 * @returns {boolean}
 */
function acceptInteraction(session, payload) {
...
module.exports = {
  ...
  HIGH_PRIORITY_PARTICIPANT_EVENTS,
  getVersion,
  readExpected,
  checkExpected,
  bump,
