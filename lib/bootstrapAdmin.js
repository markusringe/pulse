...
async function ensureBootstrapAdmin(userDb) {
  ...
  if (!password || password.length < 4) {
    console.warn("[bootstrap] BOOTSTRAP_ADMIN_PASSWORD zu kurz (min. 4 Zeichen)");
  ...
  console.log(`[bootstrap] Administrator angelegt: ${displayName} <${email}> — Anmeldung per E-Mail-PIN unter #/admin/login`);
  if (process.env.NODE_ENV !== "production") {
    console.log("[bootstrap] Entwicklung: PIN erscheint in der Dev-Mailbox oder per POST /api/auth/request-pin");
  }
