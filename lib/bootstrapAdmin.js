...
function bootstrapCredentials() {
  ...
  if (String(process.env.ADMIN_PASSWORD_HASH || "").trim()) {
    console.warn(
      "[bootstrap] ADMIN_PASSWORD_HASH wird ignoriert — Pulse erwartet BOOTSTRAP_ADMIN_PASSWORD (Klartext, serverseitig scrypt)."
    );
  }
...
/**
 * Installations-Kennwort aus .env auf DB anwenden, solange Erstlogin aussteht.
 * Behebt Abweichungen zwischen Installer, Container-Neustart und pulse.db.
 * @returns {Promise<boolean>}
 */
async function syncInstallPasswordFromEnv(userDb) {
  if (!userDb?.supported) return false;
  const { displayName, email, password, valid, envPasswordSet } = bootstrapCredentials();
  if (!valid || !envPasswordSet) return false;

  const bootstrapPending = await userService.isBootstrapPasswordLogin(userDb);
  const installUser = await Promise.resolve(userDb.findUserByEmail(email));

  if (installUser?.lastLoginAt && bootstrapPending) {
    await userDb.setSetting("bootstrapPasswordLogin", "0");
    return false;
  }

  if (!bootstrapPending && installUser?.lastLoginAt) return false;

  const needsSync =
    bootstrapPending ||
    (installUser && !installUser.lastLoginAt) ||
    (await userService.countAdmins(userDb)) === 0;
  ...
  if (synced) {
    ...
    audit.log("bootstrap_password_sync", { userId: email, action: "env_to_db" });
  }
  return synced;
}
...
async function ensureBootstrapAdmin(userDb) {
  ...
  if (adminCount > 0) {
    if (bootstrapPending) {
      const installUser = await Promise.resolve(userDb.findUserByEmail(email));
      /* Erstlogin abgeschlossen, Flag aber hängen geblieben — aufräumen. */
      if (installUser?.lastLoginAt) {
        await userDb.setSetting("bootstrapPasswordLogin", "0");
    ...
    if (!bootstrapPending) {
      ...
      if (bootstrapUser && !bootstrapUser.lastLoginAt) {
        await userDb.setSetting("bootstrapPasswordLogin", "1");
        await userDb.setSetting("onboardingBackupPending", "1");
    ...
    if (bootstrapPending) {
      ...
      await syncInstallPasswordFromEnv(userDb);
    }
...
/**
 * Installations-E-Mail oder einziger Benutzer → Rolle admin erzwingen (Reparatur bestehender VPS).
 * @param {object} userDb
 * @param {{ id?: string, email?: string, role?: string, status?: string }} user
 * @returns {Promise<object|null>} Aktualisierter DB-Benutzer
 */
async function repairInstallAdminRole(userDb, user) {
  if (!userDb?.supported || !user?.id) return null;

  const { email: installEmail } = bootstrapCredentials();
  const users = await Promise.resolve(userDb.listUsers({}));
  const isInstallEmail =
    installEmail.includes("@") && String(user.email || "").toLowerCase() === installEmail.toLowerCase();
  const isLoneUser = Array.isArray(users) && users.length === 1 && users[0].id === user.id;

  if (!isInstallEmail && !isLoneUser) return null;
  if (user.role === "admin" && user.status === "active") return null;

  const full = await Promise.resolve(userDb.findUserById(user.id));
  if (!full) return null;

  await Promise.resolve(
    userDb.updateUser(full.id, {
      ...full,
      role: "admin",
      status: "active",
    })
  ...
  audit.log("user_role_changed", {
    userId: full.id,
    action: isInstallEmail ? "install_admin_repair" : "lone_user_admin_repair",
  });
  console.log(`[bootstrap] Administrator-Rechte repariert: <${full.email}>`);
  return Promise.resolve(userDb.findUserById(full.id));
}
