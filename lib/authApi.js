...
function adminStepUpOk(auth) {
  ...
  if (auth?.user?.role === "admin") {
    /* Ohne PIN-Versand: Step-up per Code unmöglich — Kennwort-Session reicht. */
    if (!emailService.canSendPin()) return true;
    /* Erstlogin per Installations-Kennwort: kein PIN angefordert — Frontend überspringt Dialog ebenfalls. */
    if (auth.bootstrapPasswordLogin) return true;
    if (auth.pinLoginAvailable === false) return true;
  }
  return false;
}
