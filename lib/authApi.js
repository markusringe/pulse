...
function adminStepUpOk(auth) {
  ...
  /* Ohne SMTP/PIN kann kein Step-up per Code — Kennwort-Session reicht für Backups & Co. */
  if (auth?.user?.role === "admin" && !emailService.canSendPin()) return true;
  return false;
}
...
async function handleAuthApi(ctx) {
  ...
  if (req.method === "GET" && parts[1] === "auth" && parts[2] === "me") {
    ...
    send(res, 200, {
      ...
      nav: auth.user ? permissions.navForRole(auth.user.role) : permissions.navForRole("admin"),
  ...
  if (parts[1] === "users") {
    ...
    const adminCount = await userService.countAdmins(userDb);
    const bootstrap = adminCount === 0 && auth.viaSecret;
    const allowed = permissions.canManageUsers(auth.user) || bootstrap || auth.viaSecret;
