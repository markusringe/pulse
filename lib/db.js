...
function createSqliteDb() {
  ...
  try {
    ...
    db.exec(`
      CREATE TABLE IF NOT EXISTS _pulse_health (
        id INTEGER PRIMARY KEY CHECK (id = 1),
        ts INTEGER NOT NULL
      );
    `);
    const healthWrite = db.prepare("INSERT OR REPLACE INTO _pulse_health (id, ts) VALUES (1, ?)");
    const healthRead = db.prepare("SELECT ts FROM _pulse_health WHERE id = 1");

    return {
      kind: "sqlite",
      /** Lesen/Schreiben für Readiness (ohne Session-Daten). */
      healthCheck() {
        const t0 = Date.now();
        try {
          const ts = Date.now();
          healthWrite.run(ts);
          const row = healthRead.get();
          return { ok: Boolean(row && row.ts === ts), latencyMs: Date.now() - t0 };
        } catch {
...
function createJsonDb(file) {
  ...
  return {
    ...
    healthCheck() {
      const t0 = Date.now();
      try {
        const probe = path.join(path.dirname(file), ".pulse-health-probe");
