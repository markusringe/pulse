...
/**
 * Läuft Pulse im Docker-Container?
 * Admin-Installation ändert nur Dateien im laufenden Container — ohne Image-Rebuild
 * bleibt der alte Stand aktiv (B-010).
 * @returns {boolean}
 */
function isDockerContainer() {
  if (envBool("PULSE_ALLOW_DOCKER_ADMIN_UPDATE", false)) return false;
  if (String(process.env.RUNNING_IN_DOCKER || "").trim() === "1") return true;
  try {
...
/**
 * Deployment-Kontext für Admin-Updates (Host vs. Docker).
 * @returns {{
 *   kind: "docker" | "host",
 *   adminInstallAllowed: boolean,
 *   adminInstallBlockedReason: string,
 *   adminInstallHint: string
 * }}
 */
function getDeploymentContext() {
  const docker = isDockerContainer();
  return {
    kind: docker ? "docker" : "host",
    adminInstallAllowed: !docker,
    adminInstallBlockedReason: docker
      ? "Im Docker-Container ersetzt die Admin-Installation kein Image-Rebuild — der alte Container-Stand bleibt aktiv."
      : "",
    adminInstallHint: docker
      ? "Auf dem VPS: cd /opt/pulse && ./scripts/update-vps-ubuntu.sh --tag vX.Y.Z --yes (führt docker compose build aus)."
      : "",
  };
}

/** Wirft, wenn Admin-Dateisystem-Updates in Docker blockiert sind. */
function assertAdminFilesystemUpdateAllowed() {
  const ctx = getDeploymentContext();
  if (!ctx.adminInstallAllowed) {
    const err = new Error(ctx.adminInstallBlockedReason);
    err.code = "DOCKER_INSTALL_BLOCKED";
    err.hint = ctx.adminInstallHint;
    throw err;
  }
}

/** Konfiguriertes Repository owner/repo aus UPDATE_REPO — leer wenn nicht gesetzt. */
...
/**
 * Effektives Repo für GitHub-Abfragen: Env, gespeicherte Config, sonst Standard-Hinweis.
 * Ohne UPDATE_REPO in .env funktioniert die Prüfung trotzdem (markusringe/pulse).
 * Liest die State-Datei direkt — kein loadState(), um Rekursion mit defaultState zu vermeiden.
 */
function readStoredRepo() {
  try {
    const raw = JSON.parse(fs.readFileSync(STATE_FILE, "utf8"));
    const stored = String(raw?.config?.repo || "").trim();
    if (/^\w[\w.-]*\/[\w.-]+$/.test(stored)) return stored;
  } catch {
    /* Datei fehlt oder ungültig */
  }
  return "";
}

function effectiveRepo() {
  const env = configuredRepo();
  if (env) return env;
  const stored = readStoredRepo();
...
function updatesEnabled() {
  if (!envBool("UPDATE_ENABLED", true)) return false;
  return Boolean(effectiveRepo());
}
...
/** Basiskonfiguration ohne loadState/updatesEnabled-Rekursion. */
function emptyConfig() {
  const repo = configuredRepo() || readStoredRepo() || DEFAULT_REPO;
  return {
    enabled: envBool("UPDATE_ENABLED", true) && Boolean(repo),
...
function loadState() {
  ...
  try {
    const raw = JSON.parse(fs.readFileSync(STATE_FILE, "utf8"));
    return { ...base, ...raw, config: { ...base.config, ...(raw.config || {}) } };
  } catch {
    return base;
  }
}
...
function mapRelease(release, currentVersion, allowPrerelease) {
  ...
  return {
    ...
    isPrerelease,
    releaseUrl: release.html_url || `https://github.com/${owner}/${repoName}/releases/tag/${tag}`,
    publishedAt: release.published_at || release.created_at || null,
    releaseNotes: String(release.body || "_Keine Release-Notes._"),
    critical,
    downloadUrl: release.tarball_url || `https://api.github.com/repos/${owner}/${repoName}/tarball/${tag}`,
    tagName: tag,
    /** tag = nur Git-Tag, release = veröffentlichtes GitHub-Release */
    source: release._syntheticTag ? "tag" : "release",
  };
}

function compareSemverDesc(a, b) {
  const va = parseSemver(a)?.label || a;
  const vb = parseSemver(b)?.label || b;
...
/**
 * Synthetisches Release-Objekt für Git-Tags ohne veröffentlichtes GitHub-Release.
 * @param {object} tagRow
 * @param {string} repo
 */
function releaseFromTag(tagRow, repo) {
  const tag = String(tagRow.name || "");
  return {
    tag_name: tag,
    name: tag,
    body: `_Hinweis: Für Tag \`${tag}\` liegt noch kein GitHub-Release vor — Update über Tag/Tarball._`,
    draft: false,
    prerelease: false,
    html_url: `https://github.com/${repo}/releases/tag/${encodeURIComponent(tag)}`,
    tarball_url: tagRow.tarball_url,
    zipball_url: tagRow.zipball_url,
    published_at: null,
    _syntheticTag: true,
  };
}
...
/**
 * Höchste SemVer aus Releases und Tags wählen — /releases/latest kann hinter git push --tags zurückliegen.
 * @param {object[]} releases
 * @param {object[]} tags
 * @param {boolean} allowPrerelease
 * @param {string} repo
 */
function pickHighestReleaseCandidate(releases, tags, allowPrerelease, repo) {
  const list = Array.isArray(releases) ? releases : [];
  const tagList = Array.isArray(tags) ? tags : [];
  const releaseTagKeys = new Set(
    list
      .filter((r) => !r.draft && (allowPrerelease || !r.prerelease))
      .map((r) => String(r.tag_name || "").toLowerCase())
  );

  const candidates = list.filter((r) => !r.draft && (allowPrerelease || !r.prerelease));

  for (const tagRow of tagList) {
    const tag = String(tagRow.name || "");
    if (!parseSemver(tag)) continue;
    if (releaseTagKeys.has(tag.toLowerCase())) continue;
    candidates.push(releaseFromTag(tagRow, repo));
  }

  if (!candidates.length) return null;

  candidates.sort((a, b) => compareSemverDesc(a.tag_name, b.tag_name));
  return candidates[0];
}

/**
 * Release-Metadaten für Installation — auch wenn nur ein Git-Tag existiert.
 * @param {string} tag
 */
async function resolveReleaseForTag(tag) {
  const repo = effectiveRepo();
  try {
    return await githubFetch(`/repos/${repo}/releases/tags/${encodeURIComponent(tag)}`);
  } catch (err) {
    const msg = String(err.message || err);
    if (!msg.includes("404")) throw err;
    const tags = await githubFetch(`/repos/${repo}/tags?per_page=100`);
    const tagRow = (Array.isArray(tags) ? tags : []).find(
      (t) => String(t.name || "").toLowerCase() === String(tag).toLowerCase()
    );
    if (!tagRow) throw new Error(`Tag ${tag} nicht auf GitHub`);
    return releaseFromTag(tagRow, repo);
  }
}

/**
 * Neuestes passendes Release von GitHub laden.
 * Releases und Tags werden verglichen — git push --tags ohne GitHub-Release reicht für die Erkennung.
 * @param {boolean} allowPrerelease
 */
async function fetchLatestRelease(allowPrerelease) {
  ...
  const [releases, tags] = await Promise.all([
    githubFetch(`/repos/${repo}/releases?per_page=100`).catch(() => []),
    githubFetch(`/repos/${repo}/tags?per_page=100`).catch(() => []),
  ]);

  const data = pickHighestReleaseCandidate(releases, tags, allowPrerelease, repo);
  if (!data) throw new Error("Kein Release oder Tag auf GitHub");
...
async function checkForUpdates(opts = {}) {
  ...
  const repo = effectiveRepo();
  if (!repo) {
    const empty = { ...emptyInfo(), disabled: true, message: "Kein Update-Repository konfiguriert" };
    return opts.force ? empty : empty;
  }
  ...
  if (!updatesEnabled() && !opts.force) {
    return {
      ...emptyInfo(),
      disabled: true,
      message: "Updates deaktiviert (UPDATE_ENABLED)",
    };
  }
...
function getPublicConfig(cfg) {
  return {
    ...
    repo: effectiveRepo() || defaultRepoHint(),
  };
}
...
function saveConfig(patch) {
  ...
  if (patch.autoInstall != null) {
    /* Auto-Install im Container wäre wirkungslos ohne Image-Rebuild. */
    state.config.autoInstall = isDockerContainer() ? false : Boolean(patch.autoInstall);
  }
  if (patch.repo != null) {
    const r = String(patch.repo).trim();
    const allowed = new Set([effectiveRepo(), configuredRepo(), DEFAULT_REPO].filter(Boolean));
    if (/^\w[\w.-]*\/[\w.-]+$/.test(r) && allowed.has(r)) {
      state.config.repo = r;
    }
  }
  saveState(state);
...
async function runMigrations() {
  ...
  await runDataMigrations({ label: "update" });
}
...
async function installUpdate(meta = {}) {
  ...
  assertAdminFilesystemUpdateAllowed();
  installLock = true;
  const state = loadState();
  ...
  try {
    ...
    const repo = effectiveRepo();
    const release = await resolveReleaseForTag(tag);
    ...
    await execFileAsync("npm", ["install"], { cwd: ROOT, timeout: 300000 });

    setProgress(state, "installing", 65, "Frontend-CSS wird kompiliert…");
    await execFileAsync("npm", ["run", "css:build"], { cwd: ROOT, timeout: 180000 }).catch((err) => {
      console.warn("[update] css:build:", err.message || err);
    });
    await execFileAsync("npm", ["prune", "--omit=dev"], { cwd: ROOT, timeout: 120000 });

    setProgress(state, "installing", 75, "Datenbank-Migrationen…");
    await runMigrations();

    setProgress(state, "installing", 85, "Sendmail prüfen…");
    try {
      const sendmailSetup = require("./sendmailSetup");
      await sendmailSetup.ensureSendmailForPulse({ allowInstall: true });
    } catch (err) {
      console.warn("[update] Sendmail-Setup:", err.message || err);
    }
    ...
    /* Cache invalidieren, damit nach Neustart die neue Version ohne „Update verfügbar“ erscheint. */
    state.cachedInfo = null;
    state.lastCheckAt = null;
    setProgress(state, "completed", 100, "Installation abgeschlossen — Neustart folgt…");
    saveState(state);
...
async function requestGracefulRestart(reason = "update") {
  const state = loadState();
  setProgress(state, "restarting", 100, "Server startet neu…");
  await sleep(400);
...
async function onServerBoot() {
  ...
  const wasRestart =
    Boolean(state.pendingRestart) ||
    ["completed", "restarting", "failed", "downloading", "installing", "pending"].includes(phase);
  if (wasRestart) {
  ...
  try {
    ...
    if (cfg.autoInstall && info.hasUpdate && info.critical && getDeploymentContext().adminInstallAllowed) {
...
/**
 * Wartungszustand für Readiness — Update/Neustart blockiert Traffic.
 * @returns {{ updating: boolean, pendingRestart: boolean, status: string, blocksReadiness: boolean }}
 */
function getMaintenanceState() {
  const state = loadState();
  const status = String(state.progress?.status || "idle");
  const busyStatuses = new Set(["pending", "downloading", "installing", "restarting"]);
  const updating = installLock || busyStatuses.has(status);
  const pendingRestart = Boolean(state.pendingRestart);
  return {
    updating,
    pendingRestart,
    status,
    blocksReadiness: updating || pendingRestart,
  };
}

function isMaintenanceBusy() {
  return getMaintenanceState().blocksReadiness;
}
