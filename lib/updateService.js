 ...
 * Fallback auf Git-Tags, wenn noch kein Release veröffentlicht wurde.
 * @param {boolean} allowPrerelease
 */
async function fetchLatestRelease(allowPrerelease) {
  ...
  let data = null;
  try {
    data = await githubFetch(`/repos/${repo}/releases/latest`);
  } catch (err) {
    const msg = String(err.message || err);
    if (!msg.includes("404")) throw err;
    /* Kein Release — neuesten SemVer-Tag verwenden (z. B. nur git push --tags). */
    const tags = await githubFetch(`/repos/${repo}/tags`);
    if (!Array.isArray(tags) || !tags.length) throw new Error("Kein Release oder Tag auf GitHub");
    /* Höchste SemVer-Version wählen — GitHub liefert Tags alphabetisch, nicht nach Version. */
    const semverTags = tags.filter((t) => parseSemver(t.name));
    semverTags.sort((a, b) => (semverGt(a.name, b.name) ? -1 : semverGt(b.name, a.name) ? 1 : 0));
    const tagRow = semverTags[0] || tags[0];
    const tag = String(tagRow.name || "");
    data = {
      tag_name: tag,
      name: tag,
      body: `_Hinweis: Für diesen Tag wurde noch kein GitHub-Release veröffentlicht. Tag \`${tag}\` wird als Update-Quelle genutzt._`,
      draft: false,
      prerelease: false,
