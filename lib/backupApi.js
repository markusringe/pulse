...
async function handleBackupsApi(ctx) {
  ...
  if (req.method === "GET" && action === "download" && parts[3]) {
    try {
      ...
      sendZipFile(res, 200, filePath, filename, corsHeaders, req);
    } catch (err) {
      send(res, 400, { error: String(err.message || err) });
    }
    return true;
...
function sendZipFile(res, status, filePath, filename, corsHeaders, req) {
  const stat = fs.statSync(filePath);
  const headers = {
    "Content-Type": "application/zip",
    "Content-Length": String(stat.size),
    "Content-Disposition": `attachment; filename="${filename}"`,
    ...(typeof corsHeaders === "function" ? corsHeaders(req) : corsHeaders || {}),
  };
