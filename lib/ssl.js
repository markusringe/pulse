...
const proxyCert = require("./sslProxyCert");
...
function publicRow(row) {
  ...
  return {
    ...
    filesPresent,
    managedBy: row.managedBy || "pulse",
    readOnly: Boolean(row.readOnly),
    issuer: row.issuer || "",
  };
}

/** Öffentliche Zeile für das nginx-Terminierungs-Zertifikat (nur Anzeige). */
function publicProxyRow(meta) {
  const status = meta.expiresAt <= Date.now() ? "expired" : "active";
  return {
    domain: meta.domain,
    email: "",
    status,
    error: "",
    issuedAt: meta.issuedAt || 0,
...
function listCertificates() {
  const managed = store.list().map((row) => {
    const status = util.deriveStatus(row);
  ...
  const proxy = proxyCert.readProxyCertificate();
  if (!proxy) return managed;
  const front = publicProxyRow(proxy);
  /* Gleiche Domain nicht doppelt — nginx-Zertifikat ist das öffentlich sichtbare. */
  const rest = managed.filter((row) => row.domain !== proxy.domain);
  return [front, ...rest];
}

function httpsInfo() {
  const proxy = proxyCert.readProxyCertificate();
  const listening = Boolean(httpsServer && httpsServer.listening);
  return {
    listening,
    port: httpsPort(),
    httpPort: httpPort(),
    ...
    /* nginx terminiert TLS — Admin zeigt deploy/certs/fullchain.pem, kein zweiter ACME-Antrag nötig. */
    terminator: proxy ? "nginx" : listening ? "pulse" : "none",
    proxyCertPath: proxy ? proxyCert.proxyCertPath() : "",
    publicTls: proxy
      ? { active: proxy.expiresAt > Date.now(), domain: proxy.domain, port: 443 }
      : { active: listening, domain: "", port: httpsPort() },
  };
}
...
/**
 * Erkennt HTTPS auch hinter Reverse-Proxy (nginx: X-Forwarded-Proto).
 * Ohne das entsteht hinter TLS-Terminierung eine Redirect-Schleife (ERR_TOO_MANY_REDIRECTS).
...
function shouldRedirectHttp(url) {
  ...
  if (url.pathname === "/api/health" || url.pathname.startsWith("/api/health/")) return false;
  if (url.pathname === "/metrics") return false;
  return true;
}
