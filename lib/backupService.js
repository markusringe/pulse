...
/** Backup-ZIP validieren (Metadaten + pulse.db). */
async function validateBackupZip(zipPath) {
  ...
  try {
    ...
    if (!fs.existsSync(dbPath)) throw new Error("Ungültiges Backup: Datenbank fehlt");
    return { valid: true, metadata, createdAt: metadata.createdAt, appVersion: metadata.appVersion };
  } finally {
    fs.rmSync(tempDir, { recursive: true, force: true });
  }
}
...
/**
 * Aus ZIP wiederherstellen (überschreibt data/ und optional .env).
...
async function restoreFromBackup(backupPath) {
  ...
  try {
    ...
    if (!fs.existsSync(metadataPath)) throw new Error("Ungültiges Backup: Metadaten fehlen");
    const metadata = JSON.parse(fs.readFileSync(metadataPath, "utf8"));
    console.log("[backup] Restore von:", metadata.createdAt, "App:", metadata.appVersion);
