/**
 * Instanz-Branding (Standard-Preset). Liegt in data/branding.json.
 *
...
/**
 * Standard-Markenfarben (Preset, WCAG-geprüft in docs/contrast.md):
 * - primary #007CC1 — Links/Buttons
 * - secondary #F99700 — Akzent (nicht als Text auf Weiß)
 * - bg #FFFFFF, text #1A171B
 */
const BRAND_DEFAULTS = {
  ...
  text: "#1A171B",
  logo: "",
  footerText: "© 2026 Musterorganisation",
  impressumUrl: "#/impressum",
  privacyUrl: "#/privacy",
  privacyExtra:
    "Pulse ist ein Angebot der verantwortlichen Organisation. Ergänzende Hinweise können in der zentralen Datenschutzerklärung der Organisation stehen.",
  languages: ["de", "en", "fr"],
  retentionDays: 30,
  homepageUrl: "",
  wordFilter: true,
...
function sanitizeRecord(src = {}, opts = {}) {
  const base = opts.base && typeof opts.base === "object" ? opts.base : BRAND_DEFAULTS;
  const next = { ...BRAND_DEFAULTS, ...base };
  delete next.social;
...
function load() {
  try {
    ...
    const { branding } = sanitizeRecord(parsed, { base: BRAND_DEFAULTS });
    return branding;
  } catch {
...
module.exports = {
  ...
  save,
  BRAND_DEFAULTS,
  /** @deprecated Alias — bitte BRAND_DEFAULTS verwenden */
  SAARBRUECKEN: BRAND_DEFAULTS,
  DEFAULT_APP_NAME,
  sanitizeHomepageUrl,
  sanitizeAppName,
