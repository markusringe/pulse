...
function ensureInteraction(slide, opts = {}) {
  ...
  if (slide.interaction && typeof slide.interaction === "object") {
    /* In-place — Referenzen aus applyAction/canAcceptInput bleiben gültig. */
    Object.assign(slide.interaction, empty(slide.interaction));
    return slide;
  }
  if (opts.legacy) {
...
function normalizeSlide(slide) {
  if (!slide || !isInteractiveType(slide.type)) return slide;
  if (slide.interaction && typeof slide.interaction === "object") {
    /* In-place normalisieren — verhindert stale ix nach effectiveState/publicSnapshot. */
    Object.assign(slide.interaction, empty(slide.interaction));
  }
  return slide;
}
...
function finalizeSlide(slide, reason, now = Date.now()) {
  if (!slide?.interaction || !isInteractiveType(slide.type)) return;
  normalizeSlide(slide);
  /* Nur überspringen wenn bereits persistiert — effectiveState „ended“ allein reicht nicht. */
  if (slide.interaction.state === "ended") return;
  slide.interaction.state = "ended";
  slide.interaction.endedAt = now;
...
/** Timer abgelaufen — serverseitig finalisieren (endsAt absolut). */
function onTimerExpired(slide, now = Date.now()) {
  if (!slide?.interaction) return null;
  normalizeSlide(slide);
  const ix = slide.interaction;
  if (ix.state === "ended") return publicSnapshot(slide, now);
  if (ix.state !== "running" || ix.endsAt == null || ix.endsAt > now) return null;
  finalizeSlide(slide, "timeout");
  return publicSnapshot(slide, now);
}
