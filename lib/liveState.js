...
function applyFanoutEnvelope(session, envelope) {
  ...
  if (envelope.type === "slide") {
    if (payload.stateVersion != null) sessionVersion.mergeRemote(session, payload.stateVersion);
  ...
  if (envelope.type === "session" && payload.session) {
    ...
    if (incoming.stateVersion != null) sessionVersion.mergeRemote(session, incoming.stateVersion);
