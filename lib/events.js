...
const { sanitizeStageEffect, sanitizeStageEffectIntensity } = require("./stageEffectMeta");
const {
  sanitizeSpecialSlideConfig,
  sanitizeCurrentSpecialSlide,
} = require("./eventSpecialSlides");
...
function deriveStatus(ev, now = Date.now()) {
  ...
  /* Manuell beendet (z. B. Endfolie) — vor Kalenderregeln, damit Teilnahme gesperrt bleibt. */
...
function sanitizeEvent(src = {}, opts = {}) {
  ...
  return {
    ...
    countdownStyle: sanitizeCountdownStyle(
      Object.prototype.hasOwnProperty.call(src, "countdownStyle") ? src.countdownStyle : prev.countdownStyle
    ),
    showStageDateTime:
      Object.prototype.hasOwnProperty.call(src, "showStageDateTime")
        ? src.showStageDateTime !== false
        : prev.showStageDateTime !== false,
    showStageQr: Boolean(
      Object.prototype.hasOwnProperty.call(src, "showStageQr") ? src.showStageQr : prev.showStageQr
    ),
    stageEffect: sanitizeStageEffect(
      Object.prototype.hasOwnProperty.call(src, "stageEffect") ? src.stageEffect : prev.stageEffect
    ),
    stageEffectIntensity: sanitizeStageEffectIntensity(
      Object.prototype.hasOwnProperty.call(src, "stageEffectIntensity")
        ? src.stageEffectIntensity
        : prev.stageEffectIntensity
    ),
    startSlide: sanitizeSpecialSlideConfig(
      Object.prototype.hasOwnProperty.call(src, "startSlide") ? src.startSlide : prev.startSlide,
      "start"
    ),
    pauseSlide: sanitizeSpecialSlideConfig(
      Object.prototype.hasOwnProperty.call(src, "pauseSlide") ? src.pauseSlide : prev.pauseSlide,
      "pause"
    ),
    endSlide: sanitizeSpecialSlideConfig(
      Object.prototype.hasOwnProperty.call(src, "endSlide") ? src.endSlide : prev.endSlide,
      "end"
    ),
    currentSpecialSlide: sanitizeCurrentSpecialSlide(
      Object.prototype.hasOwnProperty.call(src, "currentSpecialSlide")
        ? src.currentSpecialSlide
        : prev.currentSpecialSlide
    ),
  };
}
...
function eventMetaFor(eventId) {
  ...
  return {
    ...
    countdownStyle: sanitizeCountdownStyle(ev.countdownStyle),
    showStageDateTime: ev.showStageDateTime !== false,
    showStageQr: Boolean(ev.showStageQr),
    stageEffect: sanitizeStageEffect(ev.stageEffect),
    stageEffectIntensity: sanitizeStageEffectIntensity(ev.stageEffectIntensity),
    startSlide: ev.startSlide,
    pauseSlide: ev.pauseSlide,
    endSlide: ev.endSlide,
    currentSpecialSlide: sanitizeCurrentSpecialSlide(ev.currentSpecialSlide),
    /* Effektivstatus inkl. Start-/Enddatum — konsistent mit participantEventGate. */
    status: deriveStatus(ev),
  };
}

/**
 * Live-Metadaten für Countdown/Stage patchen (Presenter-Toggles, Startzeit).
 * @param {string} eventId
 * @param {object} patch
 */
function patchEventMeta(eventId, patch = {}) {
  const store = readStore();
  const idx = store.events.findIndex((e) => e.id === eventId);
  if (idx < 0) return null;
  const prev = store.events[idx];
  const merged = { ...prev, ...patch };
  const next = sanitizeEvent(merged, { prev, allowPast: prev.status === "active" || prev.status === "ended" });
