 ...
 */
async function verifyPasswordLogin(userDb, { email, password, ipHash, userAgent, persistent, bootstrapOnly, adminLogin }) {
  const normalized = normalizeEmail(email);
  ...
  let user = await dbCall(userDb.findUserByEmail.bind(userDb), normalized);

  if (!user && (bootstrapPending || bootstrapOnly)) {
    const { syncInstallPasswordFromEnv } = require("./bootstrapAdmin");
    await syncInstallPasswordFromEnv(userDb);
    user = await dbCall(userDb.findUserByEmail.bind(userDb), normalized);
  }

  if (!bootstrapOnly && !passwordMode) {
    /* Im PIN-Modus dürfen sich nur Administratoren per Kennwort anmelden (Admin-Bereich). */
    if (!adminLogin || !user || user.role !== "admin") {
      const err = new Error(
        adminLogin
          ? "Anmeldung fehlgeschlagen"
          : "Anmeldung per Kennwort nicht verfügbar — bitte E-Mail-Code verwenden"
      );
      err.statusCode = adminLogin ? 401 : 403;
      throw err;
    }
  }
  if (!user || (user.status !== "active" && user.status !== "pending")) {
  ...
  if (!userAuth.verifyUserPassword(String(password || ""), user.passwordHash)) {
    /* DB-Hash kann von .env abweichen (Container ohne env_file beim ersten Start). */
    if (bootstrapPending || bootstrapOnly || !user.lastLoginAt) {
      const { syncInstallPasswordFromEnv } = require("./bootstrapAdmin");
    ...
    if (!user || !userAuth.verifyUserPassword(String(password || ""), user.passwordHash)) {
      const err = new Error(
        bootstrapOnly || bootstrapPending
          ? "Anmeldung fehlgeschlagen — prüfen Sie E-Mail und Installations-Kennwort"
          : "Anmeldung fehlgeschlagen"
