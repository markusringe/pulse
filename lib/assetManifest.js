...
/**
 * Einzelnen Import-Pfad mit Content-Hash anreichern (intern für static + dynamic).
 * @param {string} ref Rohtext zwischen Anführungszeichen
 * @param {string} fromWebPath
 * @param {Record<string, string>} assets
 * @param {{ strictBuild?: boolean }} opts
 * @returns {string|null} Neuer Pfad oder null wenn unverändert
 */
function hashImportRef(ref, fromWebPath, assets, opts = {}) {
  const strictBuild = opts.strictBuild === true;
  if (!isRewritableLocalAssetRef(ref)) return null;
  ...
  const ext = path.posix.extname(webPath);
  if (!HASHABLE_EXTENSIONS.has(ext)) return null;
  if (!assets[webPath]) {
    if (strictBuild) {
...
function rewriteJsImports(source, fromWebPath, assets, opts = {}) {
  let out = source.replace(
    ...
    (full, pre, _q, ref) => {
      const hashed = hashImportRef(ref, fromWebPath, assets, opts);
      if (!hashed) return full;
      /* Anführungszeichen (_q) müssen erhalten bleiben — sonst ungültige ES-Module. */
      return `${pre}${_q}${hashed}${_q}`;
    },
  );

  /* Dynamische import("./mod.js") — ohne ?h= landet der Browser in max-age-86400-Cache (Stale-Bug). */
  out = out.replace(/import\s*\(\s*(["'])([^"']+)\1\s*\)/g, (full, _q, ref) => {
    const hashed = hashImportRef(ref, fromWebPath, assets, opts);
    if (!hashed) return full;
    return `import(${_q}${hashed}${_q})`;
  });
